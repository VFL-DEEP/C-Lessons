...
// numbers and compare values in our programs.  There are different types of opertors in C:
// Arithmetic operators are used for mathematical operations like addition, subtraction, multiplication and division.
// Examples: + (addition), - (subtraction), * (multiplication), / (division), % (modlus)
// Comparrrisonn operators are used to compare two values. They return either true (1) or false (0).
